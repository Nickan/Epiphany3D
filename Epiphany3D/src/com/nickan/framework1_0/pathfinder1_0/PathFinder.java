...
/**
 * My attempt to make my own 3D path finder, y-axis is ignored for now. All the codes are copy pasted from my 2D version path finder,
 * I am just changing a little here and there for the better, I wish (>_<).
 *
 * @author Nickan
 */
...
//			System.out.println(node.x + ": " + node.y);
	...
	public void setNodesType(List<Node> nodeList) {
		for (Node node: nodeList)
			nodes[node.y][node.x].type = node.type;
	}
	...
	public void setHeuristics(Node endTile) {
		for (int y = 0; y < nodes.length; ++y) {
			for (int x = 0; x < nodes[0].length; ++x) {
				nodes[y][x].reset();
				nodes[y][x].set(x, y, getHeuristic(x, y, endTile) * 10, NodeType.WALKABLE);
			}
		}
	}
