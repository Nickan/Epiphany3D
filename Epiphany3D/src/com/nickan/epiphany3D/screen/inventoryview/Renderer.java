...
public class Renderer {
	...
	public Renderer(Handler handler, SpriteBatch spriteBatch, BitmapFont arial, BitmapFont comic, ShaderProgram fontShader) {
...
//		drawLettersWithDistanceField(batch);
	...
	private void drawLettersWithDistanceField(SpriteBatch batch) {
		StatisticsHandler statsHandler = handler.player.statsHandler;
		float widthUnit = handler.widthUnit;
		float heightUnit = handler.heightUnit;
		Vector2 statsPos = handler.statsPos;
	...
	private void initialize() {
		...
		handler.addAttributeButtons.clear();
		handler.addAttributeButtons.add(handler.addButtonStr);
		handler.addAttributeButtons.add(handler.addButtonDex);
		handler.addAttributeButtons.add(handler.addButtonVit);
		handler.addAttributeButtons.add(handler.addButtonAgi);
		handler.addAttributeButtons.add(handler.addButtonWis);

		stage.getActors().addAll(handler.addAttributeButtons);
		initializeItemSlots();
	}
